...
## 概述
js是一种非常灵活的语言，理解js引擎的执行过程对我们学习javascript非常重要，但是网上讲解js引擎的文章也大多是浅尝辄止或者只局部分析，例如只分事件循环（Event Loop）或者变量提升等等，并没有全面深入的分析其中过程。所以我一直想把js执行的详细过程整理成一个较为详细的知识体系，帮助我们理解和整体认识js。
...
function bar() {
    var B_context = "Bar Execution Context";

    function foo() {
        var f_context = "foo Execution Context";
    }
...
注：进入不同的运行环境都会进入代码预编译和执行两个阶段，语法分析则在代码块加载完毕时统一检验语法

### 创建执行上下文
执行上下文可理解为当前的执行环境，与该运行环境相对应。创建执行上下文的过程中，主要做了以下三件事件，如图：
![EC](img/ec.jpg)
1. 创建**变量对象（Variable Object）**

2. 建立**作用域链（Scope Chain）**
...
这里我们在全局环境调用fun函数，创建fun执行上下文，这里为了方便大家理解，暂时不讲解作用域链以及this，如下:
```
//创建fun执行上下文,fun Execution Context简写为funEC
funEC = {
    //变量对象
    VO: {
        arguments: {a: 2,b: 3}, //这里arguments对象的写法仅方便大家理解，但是在浏览器中展示是以类数组的方式展示的
        test: <test reference>, //表示test函数的引用地址
        num: undefined
    },
    //作用域链
    scopeChain:[],
...
注：创建变量对象发生在预编译阶段，但尚未进入执行阶段，该变量对象都是不能访问的，因为此时的变量对象中的变量属性尚未赋值，值仍为undefined，只有进入执行阶段，变量对象中的变量属性进行赋值后，变量对象（Variable Object）转为活动对象（Active Object）后，才能访问，这个过程就是VO --> AO过程。
